...
func maybeReuseOpt(coder *jsonopts.Struct, opts []Options) *jsonopts.Struct {
	if len(opts) == 1 {
		o, ok := opts[0].(*jsonopts.Struct)
		if ok && o == coder {
			return coder
		}
	} else if len(opts) == 0 {
...
func MarshalEncode(out *jsontext.Encoder, in any, opts ...Options) (err error) {
	...
	mo := maybeReuseOpt(&xe.Struct, opts)
	if mo == nil {
		mo = getStructOptions()
...
func UnmarshalDecode(in *jsontext.Decoder, out any, opts ...Options) (err error) {
	...
	uo := maybeReuseOpt(&xd.Struct, opts)
	if uo == nil {
		uo = getStructOptions()
