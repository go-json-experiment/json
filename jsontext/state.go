...
// Parent strips off the last token from the remaining pointer.
...
func (s state) appendStackPointer(b []byte, where int) []byte {
	...
	for i := 1; i < s.Tokens.Depth(); i++ {
		isLast := i == s.Tokens.Depth()-1
		if isLast && where == 0 {
			return b
		}
		switch e := s.Tokens.index(i); {
		case e.isObject():
			switch {
			case isLast && where > 0 && e.Length()%2 == 0:
				return b
			case e.Length() == 0:
				return b
			default:
		...
		case e.isArray():
			switch {
			case isLast && where > 0:
				b = strconv.AppendUint(append(b, '/'), uint64(e.Length()), 10)
			case e.Length() == 0:
				return b
			default:
				b = strconv.AppendUint(append(b, '/'), uint64(e.Length()-1), 10)
			}
		}
	}
	return b
