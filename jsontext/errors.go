...
// If err is an IO error, then it is not wrapped.
//
// It takes a relative offset pos that can be resolved into
// an absolute offset using coder.OffsetAt.
//
...
func wrapSyntacticError(state interface {
	...
	if _, ok := err.(*ioError); err == io.EOF || ok {
		return err // avoid wrapping IO related errors
	}
...
func (e *SyntacticError) Error() string {
	...
	if e.Err != nil {
		...
		sb.WriteString("syntactic error")
	}
	switch {
	case e.JSONPointer != "":
