...
func TestRawMessage(t *testing.T) {
	...
	const raw = `["\u0056", null]`
	const in = `{"X":0.1,"Id":["\u0056", null],"Y":0.2}`
	const want = `{"X":0.1,"Id":["V",null],"Y":0.2}`
	err := Unmarshal([]byte(in), &data)
	if err != nil {
...
func TestDecodeInStream(t *testing.T) {
	tests := []struct {
		...
		{CaseName: Name(""), json: ` [{"a": 1} {"a": 2}] `, expTokens: []any{
			Delim('['),
			decodeThis{map[string]any{"a": float64(1)}},
			decodeThis{&SyntaxError{"missing character ',' after object or array value", 11}},
		}},
		{CaseName: Name(""), json: `{ "` + strings.Repeat("a", 513) + `" 1 }`, expTokens: []any{
			Delim('{'), strings.Repeat("a", 513),
			decodeThis{&SyntaxError{"missing character ':' after object name", 518}},
		}},
		{CaseName: Name(""), json: `{ "\a" }`, expTokens: []any{
			Delim('{'),
			&SyntaxError{"invalid escape sequence `\\a` within string", 3},
		}},
		{CaseName: Name(""), json: ` \a`, expTokens: []any{
			&SyntaxError{"invalid character '\\\\' at start of token", 1},
		}},
	}
	for _, tt := range tests {
		t.Run(tt.Name, func(t *testing.T) {
