...
func TestEncodeRenamedByteSlice(t *testing.T) {
	...
	if namedByteInSliceAsJSONArray {
		want = "[97,98,99]"
	}
	got, err = Marshal(r)
	if err != nil {
...
func TestNilMarshal(t *testing.T) {
	tests := []struct {
		...
		{Name(""), struct{ M Marshaler }{(*nilJSONMarshaler)(nil)}, `{"M":` + ternary(avoidNilPointerMethods, "null", `"0zenil0"`) + `}`},
		{Name(""), struct{ M any }{(*nilJSONMarshaler)(nil)}, `{"M":null}`},
		{Name(""), struct{ M encoding.TextMarshaler }{}, `{"M":null}`},
		{Name(""), struct{ M encoding.TextMarshaler }{(*nilTextMarshaler)(nil)}, `{"M":` + ternary(avoidNilPointerMethods, "null", `"0zenil0"`) + `}`},
...
func TestNilMarshalerTextMapKey(t *testing.T) {
	if avoidNilPointerMethods {
		t.Skip("nil pointer in map key silently treated as empty JSON string")
	}
	got, err := Marshal(map[*unmarshalerText]int{
