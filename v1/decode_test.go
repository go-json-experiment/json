...
var unmarshalTests = []struct {
	...
	{CaseName: Name(""), in: `{"X": [1,2,3], "Y": 4}`, ptr: new(T), out: T{Y: 4}, err: &UnmarshalTypeError{Value: "array", Type: reflect.TypeFor[string]()}},
	{CaseName: Name(""), in: `{"X": 23}`, ptr: new(T), out: T{}, err: &UnmarshalTypeError{Value: "number", Type: reflect.TypeFor[string]()}},
	{CaseName: Name(""), in: `{"x": 1}`, ptr: new(tx), out: tx{}},
	{CaseName: Name(""), in: `{"x": 1}`, ptr: new(tx), out: tx{}},
	{CaseName: Name(""), in: `{"x": 1}`, ptr: new(tx), err: &UnmarshalTypeError{Type: reflect.TypeFor[tx](), Err: errors.New("unknown name \"x\"")}, disallowUnknownFields: true},
	{CaseName: Name(""), in: `{"S": 23}`, ptr: new(W), out: W{}, err: &UnmarshalTypeError{Value: "number", Type: reflect.TypeFor[SS](), Struct: "W", Field: "S"}},
	{CaseName: Name(""), in: `{"T": {"X": 23}}`, ptr: new(TOuter), out: TOuter{}, err: &UnmarshalTypeError{Value: "number", Type: reflect.TypeFor[string](), Struct: "TOuter", Field: "T.X"}},
	...
	{CaseName: Name(""), in: `{"F3": -}`, ptr: new(V), out: V{F3: Number("-")}, err: &SyntaxError{msg: "invalid character '}' within number (expecting digit)", Offset: 8}},

	// raw value errors
	{CaseName: Name(""), in: "\x01 42", err: &SyntaxError{"invalid character '\\x01' looking for beginning of value", 1}},
	{CaseName: Name(""), in: " 42 \x01", err: &SyntaxError{"invalid character '\\x01' after top-level value", int64(len(" 42 "))}},
	{CaseName: Name(""), in: "\x01 true", err: &SyntaxError{"invalid character '\\x01' looking for beginning of value", 1}},
	{CaseName: Name(""), in: " false \x01", err: &SyntaxError{"invalid character '\\x01' after top-level value", int64(len(" false "))}},
	{CaseName: Name(""), in: "\x01 1.2", err: &SyntaxError{"invalid character '\\x01' looking for beginning of value", 1}},
	{CaseName: Name(""), in: " 3.4 \x01", err: &SyntaxError{"invalid character '\\x01' after top-level value", int64(len(" 3.4 "))}},
	{CaseName: Name(""), in: "\x01 \"string\"", err: &SyntaxError{"invalid character '\\x01' looking for beginning of value", 1}},
	{CaseName: Name(""), in: " \"string\" \x01", err: &SyntaxError{"invalid character '\\x01' after top-level value", int64(len(" \"string\" "))}},
	...
	// integer-keyed map errors
	{
		...
		in:       `{"abc":"abc"}`,
		ptr:      new(map[int]string),
		err:      &UnmarshalTypeError{Value: "string", Type: reflect.TypeFor[int](), Offset: 2},
	},
	{
		CaseName: Name(""),
		in:       `{"256":"abc"}`,
		...
		t.Run(tt.Name, func(t *testing.T) {
			...
			if err := checkValid(in); err != nil {
				if !equalError(err, tt.err) {
					t.Errorf("%s: checkValid error:\n\tgot:  %v\n\t      %#v\n\twant: %v\n\t      %#v", tt.Where, err, err, tt.err, tt.err)
				}
			}
			if tt.ptr == nil {
				return
...
func TestInvalidUnmarshal(t *testing.T) {
	tests := []struct {
		...
		{Name(""), `123`, new(net.IP), &UnmarshalTypeError{Value: "number", Type: reflect.TypeFor[net.IP](), Offset: 0}},
	}
	for _, tt := range tests {
		t.Run(tt.Name, func(t *testing.T) {
			...
			case !reflect.DeepEqual(gotErr, tt.wantErr):
				t.Errorf("%s: Unmarshal error:\n\tgot:  %v\n\t      %#v\n\twant: %v\n\t      %#v", tt.Where, gotErr, gotErr, tt.wantErr, tt.wantErr)
			}
		})
	}
}
...
func TestUnmarshalErrorAfterMultipleJSON(t *testing.T) {
	tests := []struct {
		...
		CaseName: Name(""),
		in:       `1 false null :`,
		err:      &SyntaxError{"invalid character ':' before next token", int64(len("1 false null "))},
	}, {
		CaseName: Name(""),
		in:       `1 [] [,]`,
		err:      &SyntaxError{"invalid character ',' at start of value", int64(len("1 [] ["))},
	}, {
		CaseName: Name(""),
		in:       `1 [] [true:]`,
		err:      &SyntaxError{"invalid character ':' after array value (expecting ',' or ']')", int64(len("1 [] [true"))},
	}, {
		CaseName: Name(""),
		in:       `1  {}    {"x"=}`,
		err:      &SyntaxError{"invalid character '=' after object name (expecting ':')", int64(len(`1  {}    {"x"`))},
	}, {
		CaseName: Name(""),
		in:       `falsetruenul#`,
		err:      &SyntaxError{"invalid character '#' within literal null (expecting 'l')", int64(len(`falsetruenul`))},
	}}
	for _, tt := range tests {
		t.Run(tt.Name, func(t *testing.T) {
