...
func TestIndentErrors(t *testing.T) {
	tests := []struct {
		...
		{Name(""), `{"X": "foo", "Y"}`, &SyntaxError{"invalid character '}' after object name (expecting ':')", int64(len(`{"X": "foo", "Y"`))}},
		{Name(""), `{"X": "foo" "Y": "bar"}`, &SyntaxError{"invalid character '\"' after object value (expecting ',' or '}')", int64(len(`{"X": "foo" `))}},
	}
	for _, tt := range tests {
		t.Run(tt.Name, func(t *testing.T) {
			slice := make([]uint8, 0)
